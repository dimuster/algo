...
public class AVLTree implements MySet {
    private class Node {
        public int key;
        public int height;
        public Node left;
    ...
    // [NOTE: переписать в один метод rotate(Node p, Node v, X)]
    private void rotateLeft(Node p, Node v) {
        Node pr = p.parent;
        Node b = v.left;
        p.right = b;
        if (b != null) {
    ...
    private void rotateRight(Node p, Node v) {
        Node pr = v.parent;
        Node b = p.right;
        v.left = b;
        if (b != null) {
    ...
    private void remove(Node v, int x) {
        ...
        if (v.left == null) {
            // [NOTE: переписать проверки более читабельно]
    ...
    private boolean find(Node v, int x) {
        if (v == null) {
            return false;
        }
        if (v.key < x) {
    ...
    // [NOTE: убрать неоправданную глобальную переменную]
    private int lvl;
